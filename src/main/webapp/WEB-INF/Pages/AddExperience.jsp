...
<div class="main-box">
<div class="add-role">
<h1 class="h1">Add Experience</h1>
</div>
<form class="forms" id="roleForm" method="post" action="javascript:void(0)">
<p class="role-1">Experience</p>
<div class="role-form">
<div>
<input type="text" class="box" id="roleName" name="experienceName" placeholder="Enter Your Experience">
...
document.getElementById("roleForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (roleId) {
      url = "<%=request.getContextPath()%>/student/editexperience";
      method = "POST"; // Change to "PUT" if backend supports it
      payload = JSON.stringify({ id: parseInt(roleId), experience: roleName });
    } else {
      url = "<%=request.getContextPath()%>/student/saveexperience";
      method = "POST";
...
async function fetchRoles() {
  try {
    const response = await fetch("<%=request.getContextPath()%>/student/getExperience");
    if (!response.ok) throw new Error(`Failed to fetch courses: ${response.status}`);

    roles = await response.json();
	//console.log("data is........exp",  roles);
    roles = roles.filter(item => item && typeof item.id === 'number' && item.experience);
    renderTable();
  } catch (error) {
...
async function deletecourse(id) {
  ...
  try {
    const url = "<%=request.getContextPath()%>/student/deleteexperience?id=" + id;
...
function editcourse(id) {
  ...
  if (role) {
    document.getElementById("roleName").value = role.experience || '';

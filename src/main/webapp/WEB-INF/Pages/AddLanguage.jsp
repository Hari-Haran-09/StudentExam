...
<div class="main-box">
<div class="add-role">
<h1 class="h1">Add Language</h1>
</div>
<form class="forms" id="roleForm" method="post" action="javascript:void(0)">
<p class="role-1">Language</p>
<div class="role-form">
<div>
<input type="text" class="box" id="roleName" name="languageName" placeholder="Enter Your Language">
...
document.getElementById("roleForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (roleId) {
      url = "<%=request.getContextPath()%>/student/editlanguage";
      method = "POST"; // Change to "PUT" if backend supports it
      payload = JSON.stringify({ id: parseInt(roleId), languageName: roleName });
    } else {
      url = "<%=request.getContextPath()%>/student/savelanguages";
      method = "POST";
    ...
    console.error(`Error ${roleId ? "updating" : "saving"} language:`, error);
...
async function fetchRoles() {
  try {
    const response = await fetch("<%=request.getContextPath()%>/student/getLanguage");
    if (!response.ok) throw new Error(`Failed to fetch languages: ${response.status}`);

    roles = await response.json();
    roles = roles.filter(item => item && typeof item.id === 'number' && item.languageName);
    renderTable();
  } catch (error) {
    console.error("Error fetching languages:", error);
    const tbody = document.querySelector("#dataTable tbody");
    tbody.innerHTML = `<tr><td colspan="3" style="color:red;">Failed to load languages</td></tr>`;
  }
}
  ...
  paginatedData.forEach(item => {
    ...
    const row = `
      <tr style="background:#fff; box-shadow:0px 0px 4px #00000020;">
        ...
        <td>
          <button style="background:red; color:white; border:none; padding:4px 8px; border-radius:3px" onclick="deletelanguage(\${item.id})">Delete</button>
          <button style="background:#2178BD; color:white; border:none; padding:4px 8px; border-radius:3px" onclick="editlanguage(\${item.id})">Update</button>
        </td>
      </tr>
    `;
    tbody.innerHTML += row;
...
async function deletelanguage(id) {
  ...
  try {
    const url = "<%=request.getContextPath()%>/student/deletelanguage?id=" + id;
    ...
    alert(responseText || "Language deleted successfully");
    await fetchRoles();
  } catch (error) {
...
function editlanguage(id) {
  ...
  if (role) {
    document.getElementById("roleName").value = role.languageName || '';

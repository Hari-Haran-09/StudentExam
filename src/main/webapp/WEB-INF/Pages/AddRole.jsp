...
<p class="role-1">Role</p>
<div class="role-form">
<div>
<input type="text" class="box" id="roleName" name="role" placeholder="Enter Your Role">
...
document.getElementById("roleForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (roleId) {
      url = "<%=request.getContextPath()%>/student/editrole";
      method = "POST"; // Change to "PUT" if backend supports it
      payload = JSON.stringify({ id: parseInt(roleId), role: roleName });
    } else {
      url = "<%=request.getContextPath()%>/student/saveroles";
      method = "POST";
      payload = JSON.stringify({ role: roleName });
    }
    ...
    if (!response.ok) {
      throw new Error(`Failed to ${roleId ? "update" : "save"} role`);
    }
 
    const responseText = await response.text();
    alert(`${roleId ? "Role updated" : "Role added"} successfully`);
    ...
    console.error(`Error ${roleId ? "updating" : "saving"} role:`, error);
...
async function fetchRoles() {
  try {
    const response = await fetch("<%=request.getContextPath()%>/student/getRoles");
    if (!response.ok) throw new Error(`Failed to fetch roles: ${response.status}`);
 
    roles = await response.json();
    roles = roles.filter(item => item && typeof item.id === 'number' && item.role);
    renderTable();
  } catch (error) {
    console.error("Error fetching roles:", error);
    const tbody = document.querySelector("#dataTable tbody");
    tbody.innerHTML = `<tr><td colspan="3" style="color:red;">Failed to load roles</td></tr>`;
  }
}
  ...
  paginatedData.forEach(item => {
    ...
    const row = `
      <tr style="background:#fff; box-shadow:0px 0px 4px #00000020;">
        ...
        <td>\${item.role}</td>
        <td>
          <button style="background:red; color:white; border:none; padding:4px 8px; border-radius:3px" onclick="deleterole(\${item.id})">Delete</button>
          <button style="background:#2178BD; color:white; border:none; padding:4px 8px; border-radius:3px" onclick="editrole(\${item.id})">Update</button>
        </td>
      </tr>
    `;
    tbody.innerHTML += row;
...
console.log("id data is.........."+id);
  try {
    const url = "<%=request.getContextPath()%>/student/deleterole?id=" + id;
    ...
    const responseText = await response.text();
    alert(responseText || "Role deleted successfully");
    await fetchRoles();
  } catch (error) {
